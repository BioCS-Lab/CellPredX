...
class Net_encoder(nn.Module):
    def __init__(self, input_size, n_latent):
        ...
        self.nb_head = n_latent
        self.input_size = input_size
        self.attention= nn.Sequential(nn.Linear(n_latent, 64),
            nn.BatchNorm1d(64),
            nn.Tanh(),)
    ...
    def forward(self, data):
        ...
        # DACL attention
        #x_flat = torch.flatten(embedding, 1)
        E = self.attention(embedding)
        A = self.attention_heads(E).reshape(-1, self.nb_head, 2).softmax(dim=-1)[:, :, 1]
        return embedding, A
